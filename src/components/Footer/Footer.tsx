...
import { FOOTER_NAVIGATION, HOME_PAGE_PATH } from '@utils/constants';
      ...
      <Container className="flex flex-col gap-9 text-gray-200">
            ...
            {INFOS.map(info => (
              <li className={'flex gap-3 items-center'} key={info.value}>
                ...
                <div className="flex flex-col">
                  <p className="font-semibold text-[16px] leading-[16px] tracking-[-0.02em] text-gray-200 mb-0.5">
                    {info.label}
                  </p>
        ...
        <ul className="text uppercase flex gap-6 md:flex-row flex-col items-center italic font-light text-base leading-[20px]">
          {FOOTER_NAVIGATION.map(nav => (
            <li key={nav.href}>
              <Link href={nav.href} aria-label={nav.label}>
                {nav.label}
              </Link>
            </li>
          ))}
